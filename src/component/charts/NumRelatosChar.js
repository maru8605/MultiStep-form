...
import { nRelatosOptions } from '../../data/form';

const NumRelatosChar = () => {
  const chartData = useRecoilValue(getDataCharts);
  const data = chartData.nrelatos;

  const serieData = Object.keys(data).map((key) => {
    return {
      name: optionLabel(key, nRelatosOptions),
      y: data[key],
    };
  });
