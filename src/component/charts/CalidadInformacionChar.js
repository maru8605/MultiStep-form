...
import { calidadInformacionOptions } from '../../data/form';

const CalidadInformacionChar = () => {
  const chartData = useRecoilValue(getDataCharts);
  const data = chartData.calidadInformacion;

  const serieData = Object.keys(data).map((key) => {
    return {
      name: optionLabel(key, calidadInformacionOptions),
      y: data[key],
    };
  });
  ...
  const options = {
    ...
    title: {
      text: 'Cantidad y calidad de la información proporcionada',
      align: 'left',
      verticalAlign: 'top',
      y: 50,
      style: {
...
export default CalidadInformacionChar;
