...
const SatAtencionEscGeneroChar = () => {
  const chartData = useRecoilValue(getDataCharts);
  const data = chartData['satisfAtencion-escuelaDeGenero'];
  ...
  const options = {
    ...
    title: {
      text: 'Nivel de satisfacción del servicio de atención - Escuela de Género',
      align: 'left',
      verticalAlign: 'top',
      y: 50,
      style: {
    ...
    series: [
      {
        name: 'Nivel de satisfacción del servicio de atención - Escuela de Género',
        colorByPoint: true,
        type: 'pie',
        innerSize: '50%',
        data: serieData,
...
export default SatAtencionEscGeneroChar;
