...
const SatAtencionEqMedicosChar = () => {
  const chartData = useRecoilValue(getDataCharts);
  const data = chartData['satisfAtencion-equiposMedicos'];
  ...
  const options = {
    ...
    title: {
      text: 'Nivel de satisfacción del servicio de atención - Equipos Médicos',
      align: 'left',
      verticalAlign: 'top',
      y: 50,
      style: {
    ...
    series: [
      {
        name: 'Nivel de satisfacción del servicio de atención - Equipos Médicos',
        colorByPoint: true,
        type: 'pie',
        innerSize: '50%',
        data: serieData,
...
export default SatAtencionEqMedicosChar;
